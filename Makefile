CXX := clang++
CPPFLAGS_RELEASE := -O3 -Wall -Wextra -Wpedantic -Wno-missing-field-initializers -std=c++23 -Iinclude
CPPFLAGS_DEBUG := -g -Wall -Wextra -Wpedantic -Wno-missing-field-initializers -std=c++23 -Iinclude
...
perf:
	...
	@python3 perf/relayout.py perf/layouts/*.json > /dev/null
...
fixtures:
	@echo "Building fixture compiler (Sulla)..."
	@$(CXX) $(SUITE_CPPFLAGS) $(ENGINE_SRCS) tools/build_fixtures.cpp -o tools/build_fixtures $(SUITE_LDFLAGS)
	@echo "Compiling every layout in layouts/ into parts/..."
	@for f in layouts/*.json; do ./tools/build_fixtures "$$(basename "$$f" .json)"; done
	@echo "Done: parts/ built from layouts/."
